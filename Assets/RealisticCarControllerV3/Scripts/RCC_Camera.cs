...
public class RCC_Camera : MonoBehaviour{
	...
	void LateUpdate (){
		...
		if(Input.GetKeyDown(RCC_Settings.Instance.changeCameraKB)|| Input.GetButtonDown("Fire2"))
        {
			ChangeCamera();
		}
	...
	void FPS(){
		...
		if(transform.parent != hoodCam.transform){
			...
			transform.position = new Vector3(hoodCam.transform.position.x-0.05f, hoodCam.transform.position.y+0.3f, hoodCam.transform.position.z  ); ;
